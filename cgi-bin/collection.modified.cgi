...
sub validate_args
{
    if ($Args->{'action'} && ($Args->{'action'} =~ m/^(overview|show_host|show_plugin|show_type|show_graph|hostlist_json|pluginlist_json)$/))
    {
        $Args->{'action'} = $1;
    }
    else
...
sub list_hosts
{
    ...
    print qq(<div id="hosts-container" class="ui-widget-content ui-corner-bottom "><input title="Filter by host" type="text" class="title" id="host-filter"/><ul>\n);
    for (my $i = 0; $i < @hosts; $i++)
    {
...
sub action_show_host
{
    ...
    print qq(<div class="ui-widget-header ui-corner-top"><h3>Available Plugins</h3></div>\n);
    print qq(<div id="plugin-container" class="ui-widget-content ui-corner-bottom  "><ul>\n);
...
sub action_show_graph
{
    ...
    for (my $i = 0; $i < @DataDirs; $i++)
    {
        ...
        RRDs::graph ('-', '-a', 'PNG', '-s', $start_time, '-t', $short_title, @RRDDefaultArgs, @rrd_args);
